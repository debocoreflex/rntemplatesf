...
export async function searchContactsComplex(queryId, query, successCallback, errorCallback) {
  let match;
  const queryParts = query.split(/ /);
  const queryFirst = queryParts.length === 2 ? queryParts[0] : query;
  const queryLast = queryParts.length === 2 ? queryParts[1] : query;

  // Build match string for FirstName OR LastName
  match = `{contacts:FirstName}:${queryFirst}* OR {contacts:LastName}:${queryLast}*`;

  // Create a match query for full-text search
  const matchQuerySpec = smartstore.buildMatchQuerySpec(
    null,               // path — null for full-text search
    match,              // match string
    "ascending",        // order
    50,                 // page size
    "LastName"          // order path
  ...
  const querySuccessCB = (results) => {
    // Simulate filtering on "Department = 'Engineering'"
    const filtered = results.filter(
      (entry) => entry.Department === 'Engineering'
    );
    successCallback(filtered, queryId);
  };

  const queryErrorCB = (error) => {
    console.error("SmartStore Query Error:", error);
    errorCallback(error);
  };
