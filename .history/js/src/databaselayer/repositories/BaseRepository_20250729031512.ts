...
export abstract  class BaseRepository<T> implements IWrite<T>, IRead<T>, IQuery<T>,IDatabaseRead<T> {
    ...
    getById(id: string): Promise<T | null> {
     ...
     async fetchSingle(params: GenericQueryParams): Promise<T | null> {
    // Here you can use type or soupName to route
    if (params.soupName === 'users') {
      return await getUserDataDB(params) as T;
    }

    // You can route more soups later here
    return null;
  }
